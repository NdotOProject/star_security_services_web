import React, { useState, useEffect, useRef } from "react";
import { parsePath, useNavigate, useParams } from "react-router-dom";
import { Form, Button } from "react-bootstrap";

import Authenticated from "../../../../components/Authenticated";
import Table from "../../../../components/table/Table";
import httpClient from "../../../../utils/HttpClient";
import userService from "../../../../utils/UserService";

import routes from "../../../../configurations/routes";

import "./page.css";

const emptyEmployee = {
  id: undefined,
  name: {
    value: "",
    error: "",
  },
  address: {
    value: "",
    error: "",
  },
  contactNumber: {
    value: "",
    error: "",
  },
  code: {
    value: "",
    error: "",
  },
  branchId: {
    value: "",
    error: "",
  },
  departmentId: {
    value: "",
    error: "",
  },
  educationalQualificationId: {
    value: "",
    error: "",
  },
  gradeId: {
    value: "",
    error: "",
  },
  roleId: {
    value: "",
    error: "",
  },
};

const emptyAchievement = {
  description: {
    value: "",
    error: "",
  },
  name: {
    value: "",
    error: "",
  },
};

export default function Page() {
  const navigate = useNavigate();
  const { id } = useParams();
  const employeeInfo = useRef(emptyEmployee);

  const role = userService.getUserRoleId();

  const [employee, setEmployee] = useState(emptyEmployee);
  const [achievement, setAchievement] = useState(emptyAchievement);
  const [achievements, setAchievements] = useState([]);
  const [branches, setBranches] = useState([]);
  const [departments, setDepartments] = useState([]);
  const [educationalQualifications, setEducationalQualifications] = useState(
    []
  );
  const [grades, setGrades] = useState([]);
  const [roles, setRoles] = useState([]);

  const employeeEndpoint = httpClient.endpoints.employees;

  const canEditable = () => {
    return role === "24722a9d-ad40-4324-a044-50825421cc6c";
  };

  const fetchDepartments = async (branchId) => {
    const response = await httpClient.endpoints.branches.departments.get({
      pathParams: {
        id: branchId,
      },
    });

    setDepartments(response.data);
  };

  useEffect(() => {
    const fetchAchievements = async (employeeId) => {
      const response = await httpClient.endpoints.employees.achievements.get({
        pathParams: {
          id: employeeId,
        },
      });

      setAchievements(response.data);
    };

    const fetchEmployee = async () => {
      const response = await employeeEndpoint.single.get({
        pathParams: {
          id: id,
        },
      });

      const e = response.data;

      employeeInfo.current = {
        id: e.id,
        name: {
          value: e.name,
          error: "",
        },
        address: {
          value: e.address,
          error: "",
        },
        contactNumber: {
          value: e.contactNumber,
          error: "",
        },
        code: {
          value: e.code,
          error: "",
        },
        branchId: {
          value: e.department.branch.id,
          error: "",
        },
        departmentId: {
          value: e.department.id,
          error: "",
        },
        educationalQualificationId: {
          value: e.educationalQualification.id,
          error: "",
        },
        gradeId: {
          value: e.grade.id,
          error: "",
        },
        roleId: {
          value: e.role.id,
          error: "",
        },
      };

      setEmployee(employeeInfo.current);
      fetchAchievements(employeeInfo.current.id);
      fetchDepartments(employeeInfo.current.branchId.value);
    };

    const fetchBranches = async () => {
      const response = await httpClient.endpoints.branches.list.get();

      setBranches(response.data);
    };

    const fetchEducationalQualifications = async () => {
      const response =
        await httpClient.endpoints.educationalQualifications.list.get();

      setEducationalQualifications(response.data);
    };

    const fetchGrades = async () => {
      const response = await httpClient.endpoints.grades.list.get();

      setGrades(response.data);
    };

    const fetchRoles = async () => {
      const response = await httpClient.endpoints.roles.list.get();

      setRoles(response.data);
    };

    fetchEmployee();
    fetchBranches();
    fetchEducationalQualifications();
    fetchGrades();
    fetchRoles();
  }, [id, employeeEndpoint]);

  return (
    <Authenticated>
      <div className="page_container">
        <div className="page_header">
          <span>Employee information</span>
        </div>
        <div className="page_body employee_detail_page">
          <div className="form_container form-employee">
            <Form>
              <div className="account_info_section">
                <Form.Group className="mb-3">
                  <Form.Label>Code</Form.Label>
                  <Form.Control
                    type="text"
                    value={employee.code.value}
                    disabled
                    // onChange={(e) => {
                    //   const value = e.target.value;

                    //   let error = "";

                    //   if (value === "") {
                    //     error = "Code is required.";
                    //   }

                    //   setEmployee({
                    //     ...employee,
                    //     code: { value, error },
                    //   });
                    // }}
                  />
                  <span className={"form_error"}>{employee.code.error}</span>
                </Form.Group>
              </div>
              <div className="basic_info_section">
                <div className="basic_info">
                  <Form.Group className="mb-3">
                    <Form.Label>Name</Form.Label>
                    <Form.Control
                      type="text"
                      disabled={!canEditable()}
                      value={employee.name.value}
                      onChange={(event) => {
                        const value = event.target.value;

                        let error = "";
                        if (value === "") {
                          error = "Name is required.";
                        }

                        setEmployee({
                          ...employee,
                          name: { value, error },
                        });
                      }}
                    />
                    <span className={"form_error"}>{employee.name.error}</span>
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label>Address</Form.Label>
                    <Form.Control
                      type="text"
                      disabled={!canEditable()}
                      value={employee.address.value}
                      onChange={(e) => {
                        const value = e.target.value;

                        let error = "";
                        if (value === "") {
                          error = "Address is required.";
                        }

                        setEmployee({
                          ...employee,
                          address: { value, error },
                        });
                      }}
                    />
                    <span className={"form_error"}>
                      {employee.address.error}
                    </span>
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label>Contact Number</Form.Label>
                    <Form.Control
                      type="text"
                      disabled={!canEditable()}
                      value={employee.contactNumber.value}
                      onChange={(e) => {
                        const value = e.target.value;

                        let error = "";

                        if (value === "") {
                          error = "Contact Number is required.";
                        } else if (
                          !value.match(
                            /^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/
                          )
                        ) {
                          error = "Contact Number format is invalid.";
                        }

                        setEmployee({
                          ...employee,
                          contactNumber: { value, error },
                        });
                      }}
                    />
                    <span className={"form_error"}>
                      {employee.contactNumber.error}
                    </span>
                  </Form.Group>
                </div>
                <div className="more_info">
                  <Form.Group className="mb-3">
                    <Form.Label>Educational Qualification</Form.Label>
                    <Form.Select
                      disabled={!canEditable()}
                      value={employee.educationalQualificationId.value}
                      onChange={(e) => {
                        const value = e.target.value;

                        let error = "";
                        if (value === "") {
                          error = "Educational Qualification is required.";
                        }

                        setEmployee({
                          ...employee,
                          educationalQualificationId: { value, error },
                        });
                      }}
                    >
                      <option value={""}>
                        Choose educational qualification
                      </option>
                      {educationalQualifications.map((e) => {
                        return (
                          <option key={e.id} value={e.id}>
                            {e.name}
                          </option>
                        );
                      })}
                    </Form.Select>
                    <span className={"form_error"}>
                      {employee.educationalQualificationId.error}
                    </span>
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label>Grade</Form.Label>
                    <Form.Select
                      disabled={!canEditable()}
                      value={employee.gradeId.value}
                      onChange={(e) => {
                        const value = e.target.value;

                        let error = "";
                        if (value === "") {
                          error = "Grade is required.";
                        }

                        setEmployee({
                          ...employee,
                          gradeId: { value, error },
                        });
                      }}
                    >
                      <option value={""}>Choose grade</option>
                      {grades.map((e) => {
                        return (
                          <option key={e.id} value={e.id}>
                            {e.name}
                          </option>
                        );
                      })}
                    </Form.Select>
                    <span className={"form_error"}>
                      {employee.gradeId.error}
                    </span>
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label>Role</Form.Label>
                    <Form.Select
                      disabled={!canEditable()}
                      value={employee.roleId.value}
                      onChange={(e) => {
                        const value = e.target.value;

                        let error = "";
                        if (value === "") {
                          error = "Role is required.";
                        }

                        setEmployee({
                          ...employee,
                          roleId: { value, error },
                        });
                      }}
                    >
                      <option value={""}>Choose role</option>
                      {roles.map((e) => {
                        return (
                          <option key={e.id} value={e.id}>
                            {e.name}
                          </option>
                        );
                      })}
                    </Form.Select>
                    <span className={"form_error"}>
                      {employee.roleId.error}
                    </span>
                  </Form.Group>
                </div>
              </div>
              <div className="department_info_section">
                <Form.Group className="mb-3">
                  <Form.Label>Branch</Form.Label>
                  <Form.Select
                    disabled={!canEditable()}
                    value={employee.branchId.value}
                    onChange={(e) => {
                      const value = e.target.value;

                      let error = "";
                      if (value === "") {
                        error = "Branch is required.";
                      }

                      setEmployee({
                        ...employee,
                        branchId: { value, error },
                      });

                      if (value !== "") {
                        fetchDepartments(value);
                      }
                    }}
                  >
                    <option value={""}>Choose branch</option>
                    {branches.map((e) => {
                      return (
                        <option key={e.id} value={e.id}>
                          {e.name}
                        </option>
                      );
                    })}
                  </Form.Select>
                  <span className="form_error">{employee.branchId.error}</span>
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>Department</Form.Label>
                  <Form.Select
                    disabled={employee.branchId.value === "" || !canEditable()}
                    value={employee.departmentId.value}
                    onChange={(e) => {
                      const value = e.target.value;

                      let error = "";
                      if (value === "") {
                        error = "Department is required.";
                      }

                      setEmployee({
                        ...employee,
                        departmentId: { value, error },
                      });
                    }}
                  >
                    <option value={""}>Choose department</option>
                    {departments.map((e) => {
                      return (
                        <option key={e.id} value={e.id}>
                          {e.name}
                        </option>
                      );
                    })}
                  </Form.Select>
                  <span className="form_error">
                    {employee.departmentId.error}
                  </span>
                </Form.Group>
              </div>
            </Form>
            {canEditable() && (
              <div className="btn_section">
                <div className="safe_range">
                  <Button
                    disabled={employee === emptyEmployee}
                    onClick={() => {
                      setEmployee(employeeInfo.current);
                    }}
                  >
                    Reset
                  </Button>
                  <Button
                    variant="success"
                    disabled={employee === emptyEmployee}
                    onClick={() => {
                      if (
                        employee.address.value !== "" &&
                        employee.contactNumber.value !== "" &&
                        employee.branchId.value !== "" &&
                        employee.departmentId.value !== "" &&
                        employee.educationalQualificationId.value !== "" &&
                        employee.gradeId.value !== "" &&
                        employee.name.value !== "" &&
                        employee.roleId.value !== ""
                      ) {
                        employeeEndpoint.single.put({
                          pathParams: {
                            id: employee.id,
                          },
                          body: {
                            id: employee.id,
                            address: employee.address.value,
                            contactNumber: employee.contactNumber.value,
                            departmentId: employee.departmentId.value,
                            educationalQualificationId:
                              employee.educationalQualificationId.value,
                            gradeId: employee.gradeId.value,
                            name: employee.name.value,
                            roleId: employee.roleId.value,
                          },
                        });
                      }
                    }}
                  >
                    Save
                  </Button>
                </div>
                <div className="danger_range">
                  <Button
                    variant="danger"
                    disabled={employee.id === undefined}
                    onClick={() => {
                      employeeEndpoint.single
                        .delete({
                          pathParams: {
                            id: employee.id,
                          },
                        })
                        .then((response) => {
                          if (response.status === 204) {
                            navigate(routes.admin.employees);
                          }
                        });
                    }}
                  >
                    Delete
                  </Button>
                </div>
              </div>
            )}
          </div>
          <div className="page_container achievement_container">
            <div className="page_header">
              <span>Achievements</span>
            </div>
            <div className="page_body">
              {canEditable() && (
                <div className="form_container">
                  <Form>
                    <Form.Group className="mb-3">
                      <Form.Label>Name</Form.Label>
                      <Form.Control
                        type="text"
                        disabled={!canEditable()}
                        value={achievement.name.value}
                        onChange={(event) => {
                          const value = event.target.value;

                          let error = "";
                          if (value === "") {
                            error = "Name is required.";
                          }

                          setAchievement({
                            ...achievement,
                            name: { value, error },
                          });
                        }}
                      />
                      <span className={"form_error"}>
                        {achievement.name.error}
                      </span>
                    </Form.Group>
                    <Form.Group className="mb-3">
                      <Form.Label>Description</Form.Label>
                      <Form.Control
                        as="textarea"
                        rows={3}
                        disabled={!canEditable()}
                        value={achievement.description.value}
                        onChange={(event) => {
                          const value = event.target.value;

                          let error = "";
                          if (value === "") {
                            error = "Description is required.";
                          }

                          setAchievement({
                            ...achievement,
                            description: { value, error },
                          });
                        }}
                      />
                      <span className={"form_error"}>
                        {achievement.description.error}
                      </span>
                    </Form.Group>
                  </Form>
                  <Button
                    variant="success"
                    disabled={achievement === emptyAchievement}
                    onClick={() => {
                      if (
                        achievement.name.value !== "" &&
                        achievement.description.value !== ""
                      ) {
                        httpClient.endpoints.achievements.list
                          .post({
                            body: {
                              description: achievement.description.value,
                              name: achievement.name.value,
                              ownerId: employee.id,
                            },
                          })
                          .then((response) => {
                            if (response.status === 201) {
                              setAchievement(emptyAchievement);

                              setAchievements([...achievements, response.data]);
                            }
                          });
                      }
                    }}
                  >
                    Save
                  </Button>
                </div>
              )}

              <Table
                height="70vh"
                config={{
                  name: "Name",
                  description: "Description",
                }}
                content={achievements.map((e) => ({
                  key: e.id,
                  name: e.name,
                  description: e.description,
                }))}
              />
            </div>
          </div>
        </div>
      </div>
    </Authenticated>
  );
}
